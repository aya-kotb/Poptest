...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    public class ProCamera2DPanAndZoom : BasePC2D, ISizeDeltaChanger, IPreMover
    {
		...
		List<CameraTarget> camTargets = new List<CameraTarget>();
		...
		void Update(){
            if (Input.anyKeyDown || Input.GetMouseButtonDown(0)) {
				if (ProCamera2D.Instance.isPanning) {
					AddCameraTargets ();
					ProCamera2D.Instance.isPanning = false;
					panning = Panning.EndPan;
				}
			}
		}
        ...
        protected override void OnEnable()
        {
			...
			ProCamera2D.Instance.CameraTargets [ProCamera2D.Instance.CameraTargets.Count - 1].TargetPriority = 1;
        }
        ...
        void Pan(float deltaTime)
        {
            ...
            {
                if (InsideDraggableArea(mousePos))
                {
                    ...
                    _panDelta = prevMousePositionWorldCoord - currentMousePositionWorldCoord;
                }
            }
            // Move to edges delta
            ...
            if (_panDelta != Vector2.zero)
            {
				...
				if(panning == Panning.Default || panning == Panning.EndPan){
					panning = Panning.StartPan;
					ProCamera2D.Instance.isPanning = true;
					RemoveCameraTargets();
				}
        ...
        public void AddCameraTargets()
        {
            ...
            //Debug.Log ("sdfasdasdasdasdasd --- >" + camTargets.Count);
            for (int i =0;i< camTargets.Count; i++) {
                ProCamera2D.Instance.AddCameraTarget(camTargets[i].TargetTransform);
                ProCamera2D.Instance.CameraTargets[i].TargetPriority = camTargets[i].TargetPriority;
            }
            //ProCamera2D.Instance.AddCameraTarget (_panTarget);
            camTargets.Clear();
        }
        ...
        void RemoveCameraTargets(){
            for(int i = 0; i <ProCamera2D.Instance.CameraTargets.Count; i++){
                camTargets.Add(ProCamera2D.Instance.CameraTargets[i]);
                camTargets[i].TargetPriority = ProCamera2D.Instance.CameraTargets[i].TargetPriority;
                //  Debug.Log("Remove Cam Priority --- >" + camTargets[i].TargetPriority + "name -- >"+ camTargets[i].TargetTransform.name);
            }
