...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    ...
    public class ProCamera2D : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public void Move(float deltaTime)
        {
			...
			_targetsMidPoint = GetTargetsMidpointInViewport (CameraTargets);
        ...
        Vector3 GetTargetsWeightedMidPoint(IList<CameraTarget> targets)
        {
           ...
           // return VectorHV(midPointH, midPointV);
			return GetTargetsMidpointInViewport(targets);
        }


		Vector3 GetTargetsMidpointInViewport(IList<CameraTarget> targets){
			var midPointH = 0f;
			var midPointV = 0f;
			var totalTargets = targets.Count;
			var totalInfluencesH = 0f;
			...
			var myList = targets;
			myList = myList.OrderBy (o => o.TargetPriority).ToList ();
			int currenttargetPriority = myList [0].TargetPriority;
			//			Debug.Log ("SortedElement ----- >" + myList [0].TargetPriority + "myList [0].Name -- >"+ myList [0].TargetTransform.name);

			for (int i = 0; i < targets.Count; i++) {
				if (targets [i].TargetPriority == currenttargetPriority) {
					Vector3 viewPos = UnityEngine.Camera.main.WorldToViewportPoint (targets [i].TargetPosition);
					...
					if (onScreen) {
						...
						myList = targets;
						myList = myList.OrderBy (o => o.TargetPriority).ToList ();
						//						Debug.Log ("NOT VISIBLE ------>" + myList [0].TargetPriority + "myList [0].Name -- >"+ myList [0].TargetTransform.name);
						midPointH = 0f;
						midPointV = 0f;
						totalTargets = targets.Count;
						totalInfluencesH = 0f;
						totalInfluencesV = 0f;
						totalAccountableTargetsH = 0;
						totalAccountableTargetsV = 0;
						//return transform.localPosition;
						midPointH += (Vector3H (myList [0].TargetPosition) + myList [0].TargetOffset.x) * myList [0].TargetInfluenceH;
						midPointV += (Vector3V (myList [0].TargetPosition) + myList [0].TargetOffset.y) * myList [0].TargetInfluenceV;

						totalInfluencesH += myList [0].TargetInfluenceH;
						totalInfluencesV += myList [0].TargetInfluenceV;

						if (myList [0].TargetInfluenceH > 0)
							totalAccountableTargetsH++;

						if (myList [0].TargetInfluenceV > 0)
							totalAccountableTargetsV++;
...
//					totalAccountableTargetsH = 0;
//					totalAccountableTargetsV = 0;
//					if (targets [i].TargetTransform.name.Equals ("PC2DPanTarget")) {
//						midPointH += (Vector3H (targets [targets.Count - 1].TargetPosition) + targets [targets.Count - 1].TargetOffset.x) * targets [targets.Count - 1].TargetInfluenceH;
//						midPointV += (Vector3V (targets [targets.Count - 1].TargetPosition) + targets [targets.Count - 1].TargetOffset.y) * targets [targets.Count - 1].TargetInfluenceV;
//
//						totalInfluencesH += targets [targets.Count - 1].TargetInfluenceH;
//						totalInfluencesV += targets [targets.Count - 1].TargetInfluenceV;
//
//						if (targets [targets.Count - 1].TargetInfluenceH > 0)
//							totalAccountableTargetsH++;
//
//						if (targets [targets.Count - 1].TargetInfluenceV > 0)
//							totalAccountableTargetsV++;
//
//					}
				}
